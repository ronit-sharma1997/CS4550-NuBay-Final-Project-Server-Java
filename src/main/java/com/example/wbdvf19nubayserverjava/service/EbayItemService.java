    ...
    private void parseEbayJSON(List<EbayItem> response, URL httpsGETRequest) throws IOException {
        ...
        for(JsonNode item : itemTree) {
            ...
            response.add(new EbayItem(item.path("itemId").get(0).asText(), item.path("title").get(0).asText(),
                    ...
                    item.path("sellingStatus").get(0).path("currentPrice").get(0).path("@currencyId").asText() + " " + item.path("sellingStatus").get(0).path("currentPrice").get(0).path("__value__").asText(), item.path("sellerInfo").get(0).path("sellerUserName").get(0).asText(), item.path("sellerInfo").get(0).path("positiveFeedbackPercent").get(0).asText(), conditionType));
        }
    }
